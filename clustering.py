...
def addFilesToIndex(indexObj, csvPath):
    ...
    with codecs.open(csvPath, "r", "ISO-8859-1") as csvfile:
        ...
        for row in csvreader:
            #TODO: change this to the new column from spam removal
...
def createNormalizedMatrix():
    ### Create Reader to read in csv file after spam removal,
    #  read in the column from
    ...
    addFilesToIndex(indexToImport, OUTPUT)
...
print('We done.')
# Notes from Spencer:
# Do mutual information to get words that are prominent in only 1 cluster
# how popular a word is in general, then
# of time it appears in 1 clusters / total # of times it appears in all  documents
# can weigh the times it appears time 2 to make it more important
# also look at how popular the word is in the cluster itself
# if the words are the same in all cluster, get array the can ignore them
# interesting if something like Mozilla is not in an array
# LOOK INTO MI - give info specific to cluster
# might also want to take those words out and re-cluster, might get better results
